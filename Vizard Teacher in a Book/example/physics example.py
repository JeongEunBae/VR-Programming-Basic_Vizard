...
avatar_hit  = vizact.parallel(  vizact.fade(1,0,1), vizact.call( cry.play ))
avatar_sequence = vizact.sequence([ avatar_hit, vizact.fade(0,1,1 )], 1 )


#Create the actions for popping balloons.
popping_sound = viz.addAudio('art/pop.wav')
play_pop = vizact.call( popping_sound.play )
popping_action = vizact.size([.1,.2,.1],.5,viz.TIME )
...
def reset_balloons():
	for balloon in balloons:
		balloon.addAction( vizact.size([1,1,1], .5,viz.TIME ))

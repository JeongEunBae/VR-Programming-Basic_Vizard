...
def animate_actions( kind, axis ):
	...
	arrow_actions = [ vizact.fade( WAITING_ALPHA, ACTIVE_ALPHA, .5 ),vizact.sendsignal( go_signal ),vizact.waittime(1), vizact.fade( ACTIVE_ALPHA,WAITING_ALPHA, .5 ) ]
	...
	if kind == 'POSITION':
		...
		object_actions +=  [ vizact.goto( where[0],where[1],where[2], 1 ), vizact.goto( back[0],back[1],back[2], 1) ]
	elif kind == 'ROTATE':
		where[axis] = 1 
		...
		where[axis] = 2
		object_actions += [ vizact.size( where, 1 ),  vizact.size( back, 1 ) ]

...
for kind in ['events', 'sampler']:
	...
	for i in range(1, joy.getButtonCount()+1 ):
		...
		if kind == 'events':
			...
			text.setScale(.5,.5 )
		text.alignment( viz.TEXT_CENTER_CENTER )
		text.setPosition(interval*(i-1) , 1,1 )
...
#Sample data.
def updateGets():
	sampler['Hat']['model'].setEuler( 0,0,joy.getHat() )
	...
	sampler['Twist']['model'].setEuler( joy.getTwist(),0,0 )
...
grow = vizact.parallel( vizact.size( [1,1,1], .1 , viz.TIME ), vizact.fade(viz.WHITE, viz.RED, .1 ) ) 
shrink = vizact.parallel( vizact.size( [.2,.2,.2], .1 , viz.TIME ), vizact.fade(viz.RED, viz.WHITE, .1 ) ) 
growAndShrink = vizact.sequence( [ grow, shrink ] )
highlight = vizact.sequence( [vizact.fade(viz.WHITE, viz.RED,.5), vizact.fade(viz.RED, viz.WHITE, .01)])
...
def twist( e ):
	eventer[ 'TWIST' ]['model'].setEuler([0,0,e.twist*180] )
	all_event_changer( 'TWIST', e.twist )

def joymove( e ):
	eventer[ 'MOVE' ]['model'].setPosition( [e.pos[0], e.pos[1]-MODEL_Y, e.pos[2]], viz.ABS_PARENT )
	all_event_changer( 'MOVE', e.pos )
	
def hat( e ):
	eventer[ 'HAT' ]['model'].setEuler([0,0,e.hat])

...
def animate_actions( kind ):
	...
	arrow_actions = [ vizact.fade( alphas[0], ACTIVE_ALPHA, .1 ),vizact.sendsignal( go_signal ), vizact.fade( ACTIVE_ALPHA,alphas[0], .1 ) ]
	#arrows
...
for i in range(3):
	...
	text.setScale(.30,.30,.30)
	text.setPosition(.01,i*.03+ .015)
	texts.append( text )
#Add a sentence at the top of the screen to describe current transformations.
texts[2].setPosition(.01,.965)
